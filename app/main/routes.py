...
def index():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(post)
        db.session.commit()
        flash(_('Ваш пост в прямом эфире'))
...
def edit_profile():
    ...
    return render_template('edit_profile.html', title=_('Редактировать профиль'),
                           form=form)
...
def follow(username):
    ...
    db.session.commit()
    flash(_('Вы подписанны на %(username)s!', username=username))
    return redirect(url_for('main.user', username=username))
...
def unfollow(username):
    ...
    db.session.commit()
    flash(_('Вы не подписанны на %(username)s.', username=username))
    return redirect(url_for('main.user', username=username))
